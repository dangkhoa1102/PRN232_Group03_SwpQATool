...
public class UserRepository : IUserRepository
{
    ...
    public async Task<IEnumerable<User>> GetAllAsync(string? role, bool unassigned)
    {
        ...
        if (unassigned)
            query = query.Where(u => u.Role == "Student" && !u.GroupsNavigation.Any());
    ...
    {
        return await _context.Groups.AnyAsync(g => g.Students.Any(s => s.UserId == userId));
    }
    ...
    public async Task DeleteAsync(User user)
    {
        ...
        if (string.Equals(user.Role, "Student", StringComparison.OrdinalIgnoreCase))
        {
            ...
            try
            {
                // Load group memberships and clear them
                var userWithGroups = await _context.Users
                    .Include(u => u.GroupsNavigation)
                    .FirstAsync(u => u.UserId == user.UserId);

                userWithGroups.GroupsNavigation.Clear();
                await _context.SaveChangesAsync();

                _context.Users.Remove(userWithGroups);
                await _context.SaveChangesAsync();
